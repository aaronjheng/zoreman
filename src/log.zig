...
pub const LogSink = struct {
    ...
    /// `line` should not contain a trailing newline.
    pub fn writeLine(self: *LogSink, name: []const u8, line: []const u8) !void {
...
///
/// `name` must outlive this call (caller owns).
pub fn runReader(sink: *LogSink, io: Io, src: Io.File, name: []const u8, reader_buffer: []u8) void {
    ...
    while (true) {
        ...
        sink.writeLine(name, line) catch return;
    }
}
